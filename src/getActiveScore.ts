...
export const getActiveScore = async (address: string): Promise<any> => {
  ...
  const aggregateCalldata = multicall.interface.encodeFunctionData(
    'aggregate',
    [
      tokenIdList.map((id) => {
        return {
  ...
  const resultClaimed = multicall.interface.decodeFunctionResult(
    'aggregate',
    r
  );
  const alreadyClaimedIds: any = {};
  resultClaimed.returnData.forEach((r: any, i: number) => {
    if (
      phiClaimIface
  ...
  const datas = tokenIdList.filter((_value: number, i: number) => {
    return alreadyClaimedIds[i];
  });
  const aggregateCalldata2 = multicall.interface.encodeFunctionData(
    'aggregate',
    [
      datas.map((id) => {
        return {
  ...
  const resultClaimed2 = multicall.interface.decodeFunctionResult(
    'aggregate',
    r2
  );
  let expGain = 0;
  const details: { tokenId: number; exp: number }[] = [];
